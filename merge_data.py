...
located at the parent dataset folder.

Layout example
--------------
root_dataset/
  data.yaml            # ← baseline classes live here (authoritative)
  group1/              # sub‑dataset 1
    images/{train,val,test}
    labels/{train,val,test}
  pre_train_dataset/   # sub‑dataset 2 (any name)
    images/...
    labels/...
  other_subdataset/    # sub‑dataset 3 (layout B also supported: train/images, train/labels, ...)
    train/images ...
    train/labels ...
...
Usage
-----
python3 merge_data_baseline.py \
...
- If a sub‑dataset has no data.yaml, its IDs are mapped by **names found in its labels**, if any exist,
  else their numeric IDs are ignored unless you supply a per‑dataset alias map (future feature).
...
# ---------------- Discovery & pairing (layout A and B) --------------
...
def build_baseline(src_dir: Path) -> List[str]:
    ...
    return list(base["names"])  # make a copy
...
def per_dataset_remap(ds: Dict, baseline_names: List[str]) -> Dict[int, Optional[int]]:
    ...
    If ds has no names, we can only keep numeric IDs that coincide with baseline index names
    (i.e., impossible), so we drop by default. Supply per‑dataset alias logic if needed.
    """
    remap: Dict[int, Optional[int]] = {}
    name_to_new = {n: i for i, n in enumerate(baseline_names)}

    if ds["names"]:
        # build reverse index old_id -> name
...
def merge(src_roots: List[Path], out_root: Path, baseline_names: List[str],
    ...
    report = {s: {"pairs": 0, "img_written": 0, "lab_written": 0, "skipped_pairs": 0, "dropped_anns": 0, "empty_labels": 0}
              for s in all_splits}
        ...
        for ds, remap in zip(datasets, remaps):
            ...
            for img, lab in tqdm(pairs, desc=f"{split or 'root'}: {ds['root'].name}"):
                if not img.exists() or not lab.exists():
                    if accept_missing: report[split]["skipped_pairs"] += 1; continue
                ...
                orig = sum(1 for _ in txt.splitlines() if _.strip())
                report[split]["dropped_anns"] += max(0, orig - len(new_lines))
    ...
    yaml = (
        f"# Auto-generated by merge_data_baseline.py\n"
        f"path: .\n"
        f"train: images/train\n"
        f"val: images/val\n"
        f"test: images/test\n\n"
        f"nc: {len(baseline_names)}\n"
        f"names:\n{names_block}\n"
    )
    ...
    for s, r in sorted(report.items()):
        print(
            f"  {s or '.'}: pairs={r['pairs']} imgs={r['img_written']} labels={r['lab_written']} "
...
def main():
    ...
    ap.add_argument("--src-dir", type=Path, required=True, help="Parent directory that holds sub-datasets and baseline data.yaml")
    ...
    ap.add_argument("--keep-empty-labels", dest="keep_empty_labels", action="store_true", default=True, help="Keep images even if annotations were all dropped")
    ap.add_argument("--drop-empty-labels", dest="keep_empty_labels", action="store_false", help="Drop images whose labels became empty after filtering")
    args = ap.parse_args()
